...
O frontend foi totalmente refeito com React, substituindo o HTML e JavaScript puro. A interface agora é composta por componentes reutilizáveis com Hooks, validação de domínios antes de enviar requisições e exibição clara dos dados do Whois, incluindo feedback visual de carregamento e mensagens de erro.

No backend, o código foi reorganizado em camadas (Presentation, Application, Domain e Infrastructure), com controllers simplificados, injeção de dependência via interfaces e centralização das regras de negócio nas entidades de domínio. Serviços e adapters gerenciam DNS e Whois, DTOs controlam os dados retornados, e a arquitetura facilita testes unitários e manutenção. Migrations e ferramentas foram ajustadas para garantir consistência e evitar conflitos de versão.